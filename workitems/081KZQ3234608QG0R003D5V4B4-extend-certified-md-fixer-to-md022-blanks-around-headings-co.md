---
id: 081KZQ3234608QG0R003D5V4B4
type: task
state: backlog
priority: P2
slug: extend-certified-md-fixer-to-md022-blanks-around-headings-co
title: "Extend certified md fixer to MD022 blanks-around-headings (coverage gap found by drift evolution)"
created: 2026-08-11T00:20:19.718Z
depends_on: []
composes_with: []
---

# Extend certified md fixer to MD022 blanks-around-headings (coverage gap found by drift evolution)

<!-- Work-item body. ZetaId-keyed (conflict-free, time-sortable). "Backlog" is a
     STATE = this folder; completion moves the file to workitems/done/YYYY/MM/.
     Identity is the zetaid prefix — resolve cross-refs by `081KZQ3234608QG0R003D5V4B4-*.md` glob. -->

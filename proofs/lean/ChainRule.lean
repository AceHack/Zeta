...
theorem I_zInv_eq (s : Stream G) (n : ℕ) :
  ...
  -- Skeleton: induct on `n`; expand `Finset.sum_range_succ` on both
  -- sides and cancel the head term. Mathlib lemma to cite:
  -- `Finset.sum_range_succ` + `Finsupp.sub_apply`.
  sorry
...
theorem D_I_eq (s : Stream G) : D (I s) = s := by
  -- Skeleton: for each `n`, `D (I s) n = I s n - zInv (I s) n`;
  -- expand `zInv (I s) (n+1) = I s n`; the prefix sum up to `n+1`
  -- minus the prefix sum up to `n` equals `s (n+1)` by
  -- `Finset.sum_range_succ`. Base case: `D (I s) 0 = I s 0 - 0 = s 0`.
  sorry
...
theorem linear_commute_zInv (f : Stream G → Stream H)
  ...
  -- Skeleton: case-split on `n` = 0 vs. `n+1`; `n=0` uses `hf.map_zero`.
  sorry

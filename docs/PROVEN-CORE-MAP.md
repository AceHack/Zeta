...
| 6 | **Metric / aggregation algebra** | `byte-cost`, `Bloom`/`CountMin`/`Sketch` | byte-cost ✓ · HLL+Bloom join & CMS monoid merge-laws ✓ (state-level) · error-DIRECTION ✓ (Bloom no-false-neg, CMS no-undercount); magnitude bounds: **empirical ✓** (Metric.MagnitudeBounds — CMS overestimate ≤ ε·N, Bloom FP-rate bounded; deterministic), **formal ✗** (closed-form ε/δ Lean/Z3 still open) | byte-cost ✓ (sketches F#-only) | ✗ | ✗ | ✗ | ✓ (Metric.Homeostat — Bloom OR=semilattice, CMS add=monoid; merge-convergence + error-direction preserved) | math-leg (merge + error-direction + empirical magnitude) + homeostat; **formal magnitude bounds (research), 4-ser/Arrow/Bonsai (blocked — no rehydrate-from-state ctor), 4-lang open** |
...
Only **1 of 6 is NOT yet full: metric/aggregation** (math leg only — merge + error-direction
proven; magnitude bounds + 4-ser/Arrow/Bonsai/homeostat + 4-lang open). Merkle's 4-lang leg

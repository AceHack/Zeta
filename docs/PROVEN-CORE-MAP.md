...
| 6 | **Metric / aggregation algebra** | `byte-cost`, `Bloom`/`CountMin`/`Sketch` | byte-cost ✓ · HLL+Bloom join & CMS monoid merge-laws ✓ (state-level) · error-DIRECTION ✓ (Bloom no-false-neg, CMS no-undercount); probabilistic magnitude bounds ✗ | byte-cost ✓ (sketches F#-only) | ✗ | ✗ | ✗ | ✓ (Metric.Homeostat — Bloom OR=semilattice, CMS add=monoid; merge-convergence + error-direction preserved) | math-leg + homeostat; **magnitude bounds (research), 4-ser/Arrow/Bonsai (blocked — no rehydrate-from-state ctor), 4-lang open** |

...
const PROMPT_TEMPLATE = (langName: string, htmlContent: string) => `You are an expert, native-level writer and editor in ${langName}. 
...
function processChunk(langName: string, chunkHtml: string): string {
    ...
    try {
        ...
        console.error(\`Error processing chunk:\`, error?.stderr || error?.message);
        return chunkHtml;
    }
}
...
function prepareFile(langCode: string): boolean {
    ...
    content = content.replace('<html lang="en">', LANG_HTML_TAGS[langCode]);

    // Replace langbar links
    content = content.replace(LANGBAR_MAP['en'][0], LANGBAR_MAP['en'][1]);
    content = content.replace(LANGBAR_MAP[langCode][0], LANGBAR_MAP[langCode][1]);

    if (langCode === 'zh') {
        content = content.replace(/href="\\.\\.\\//g, 'href="./');
        content = content.replace('<nav class="langbar">', '<nav style="text-align:center;font-family:var(--sans);font-size:.78rem;letter-spacing:.12em;text-transform:uppercase;color:var(--meta);padding:.9rem 0 0">');
        content = content.replace('<a href="./en/">English</a>', '<a href="en/" style="color:var(--seal);text-decoration:none">English</a>');
        content = content.replace('<a href="./id/">Indonesia</a>', '<a href="id/" style="color:var(--seal);text-decoration:none">Indonesia</a>');
        content = content.replace('<a href="./vi/">Tiếng Việt</a>', '<a href="vi/" style="color:var(--seal);text-decoration:none">Tiếng Việt</a>');
        content = content.replace('<a href="./ko/">한국어</a>', '<a href="ko/" style="color:var(--seal);text-decoration:none">한국어</a>');
        content = content.replace('<a href="./th/">ภาษาไทย</a>', '<a href="th/" style="color:var(--seal);text-decoration:none">ภาษาไทย</a>');
        content = content.replace('<a href="./ru/">Русский</a>', '<a href="ru/" style="color:var(--seal);text-decoration:none">Русский</a>');
        content = content.replace('<a href="./ar/">العربية</a>', '<a href="ar/" style="color:var(--seal);text-decoration:none">العربية</a>');
    }
...
function main() {
    ...
    for (const langCode of langsToRun) {
        if (LANGUAGES[langCode]) {
            rewriteFile(langCode, LANGUAGES[langCode]);
        } else {

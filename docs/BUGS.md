...
- **Fix (NOT a single-file drive-by — scope discovered 2026-06-06):** add a recursion-depth guard returning `Error` past a bound (Result-over-exception, fits the no-throw contract) — or trampoline. **The codec has 4-LANGUAGE parity tests + cross-oracle differential fuzzing**, so guarding only the F# codec would make F#→`Error` while C#/Rust/TS→`Ok` on the same deep input = a parity mismatch. The guard must therefore be **mirrored across all four languages** (F#/C#/Rust/TS), across **both JSON and XML** (encode + decode). A *dedicated* `TooDeep` error case needs `public-api-designer` (Ilyana) review + the 4-lang `DecodeError`/`EncodeError` mirror; reusing an existing case avoids the API change but is semantically muddy. Bound must sit above any real value depth so golden vectors are unaffected. Validate on a windows-x64 runner (NOT reproducible on darwin/arm).
- **Who:** DynamicValue owner / Kenji (+ Ilyana if a new error case) — coordinated cross-language proven-primitive change. Otto did the diagnosis; routed for the scope decision rather than landing blind across 3 unvalidatable langs.

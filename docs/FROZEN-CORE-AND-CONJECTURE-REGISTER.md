...
| **Privacy-from-identity** (distinctness ⟹ private state) | **necessity ✅ DISCHARGED 2026-06-05** (Lean, axiom-FREE); dynamics + halting open | `Privacy/IdentityForcesPrivacy.lean`: `distinctness_forces_private` PROVEN — under public convergence (commons equal), behaviorally-distinct travelers MUST differ in private state (Leibniz identity-of-indiscernibles); `key_alone_insufficient` ties to proven Identity-injectivity (distinct keys necessary, not sufficient — privacy is the state-level complement); `no_private_collapses` (no private ⟹ collapse). **Still open:** the DYNAMICS (commons actually converges — rides the proven CRDT merge) and the stronger HALTING claim (B-1019 DST experiment). Necessity formalized; dynamics/halting remain. |

...
|------------|-------|-------------|
| **Adinkra-as-generator reconstruction** (bulk-from-boundary) | **toy core ✅** + **erasure-correction principle ✅** DISCHARGED 2026-06-05 (Lean, sorry-free, axiom-audited); MDS *construction* open | `ToyModel.lean`: `reconstruction_property`/`lemma1_toy`/`code_covers_boundary` PROVEN — linear `reconstruct = id.prod G` recovers every codeword from its 12 boundary coords (graph-code of any linear G). `ErasureDistance.lean`: `erasure_correctable_of_min_distance` + `recover_from_any_12_of_16` PROVEN — a distance-`d` code recovers UNIQUELY from any `<d` erasures; distance-5 ⇒ any-4-erasure (arbitrary 12-of-16) correction. **Still open (named):** the *construction* of a concrete distance-5 `[16,12]` MDS/Reed-Solomon code (exists over `ZMod 17`, 16≤17, Singleton d=n−k+1=5) and which G the imaginary-stack mul-table induces (needs Cauchy/Vandermonde determinants); the continuous/∞-dim lift. |

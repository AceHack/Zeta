...
export interface DlaCluster {
  ...
  readonly sites: Array<[number, number]>;  // boundary sites (x, y)
...
export function generateDlaCluster(seed: number, nWalkers: number): DlaCluster {
  ...
  for (let w = 0; w < nWalkers; w++) {
    ...
    // Spawn on a circle of radius spawnR
    const angle = rng.nextF32() * 6.283185307179586;
    let x = Math.round(CENTER + spawnR * Math.fround(Math.cos(angle)));
    let y = Math.round(CENTER + spawnR * Math.fround(Math.sin(angle)));
    ...
    for (let step = 0; step < 50000; step++) {
      const dir = rng.next() & 3;
      if (dir === 0) x = Math.min(GRID_SIZE - 1, x + 1);
      else if (dir === 1) x = Math.max(0, x - 1);
      else if (dir === 2) y = Math.min(GRID_SIZE - 1, y + 1);
  ...
  // Extract boundary sites (occupied sites adjacent to at least one empty site)
  ...
  return { grid, sites: boundary, gridSize: GRID_SIZE, nWalkers };
}
...
export interface HarmonicMeasure {
  ...
  readonly sites: Array<[number, number]>;
}

export function computeHarmonicMeasure(
  cluster: DlaCluster,
  nProbes: number,
  seed: number,
): HarmonicMeasure {
  ...
  for (let p = 0; p < nProbes; p++) {
    const angle = rng.nextF32() * 6.283185307179586;
    let x = Math.round(CENTER + launchR * Math.fround(Math.cos(angle)));
    let y = Math.round(CENTER + launchR * Math.fround(Math.sin(angle)));
    x = Math.max(0, Math.min(gridSize - 1, x));
    y = Math.max(0, Math.min(gridSize - 1, y));

    // Walk until hitting the cluster or escaping
    for (let step = 0; step < 100000; step++) {
      const dir = rng.next() & 3;
      if (dir === 0) x = Math.min(gridSize - 1, x + 1);
      else if (dir === 1) x = Math.max(0, x - 1);
      else if (dir === 2) y = Math.min(gridSize - 1, y + 1);
      else y = Math.max(0, y - 1);

      const dx = x - CENTER, dy = y - CENTER;
      if (dx * dx + dy * dy > (launchR + 10) * (launchR + 10)) break;

      if (grid[y * gridSize + x] === 1) {
        // Hit the cluster — find which boundary site was approached
        const neighbors = [[x + 1, y], [x - 1, y], [x, y + 1], [x, y - 1]];
        for (const [nx, ny] of neighbors) {
          const idx = siteIndex.get(ny * gridSize + nx);
          if (idx !== undefined) {
            hits[idx]++;
            break;
          }
        }
        // Also check the hit site itself
        const selfIdx = siteIndex.get(y * gridSize + x);
        if (selfIdx !== undefined) hits[selfIdx]++;
        break;
      }
    }
  }
  ...
  const mu = new Float64Array(sites.length);
  if (total > 0) {
    for (let i = 0; i < sites.length; i++) mu[i] = hits[i] / total;
  }
...
export function computeMultifractalSpectrum(
  ...
  if (muPos.length === 0) {
    return {
      alpha: new Float64Array(0), falpha: new Float64Array(0),
      tau3: 0, alpha0: 1.71, dfBox: 1.71,
    };
  }
  ...
  for (let i = 0; i < nAlpha; i++) {
    ...
    const dq = qi > 0 && qi < tauValues.length - 1
      ? (tauValues[qi + 1] - tauValues[qi - 1]) / 1.0
      : 0;
    alpha[i] = dq;
    falpha[i] = q * dq - tauValues[qi];
    // α₀ is where f(α) is maximised (≈ D_f for DLA)
    if (i === 0 || falpha[i] > falpha[Math.floor((i - 1) * nAlpha / nAlpha)]) {
      alpha0 = alpha[i];
    }
  }
...
function computeBoxCountingDf(cluster: DlaCluster): number {
  ...
  for (const b of boxSizes) {
    let count = 0;
    for (let y = 0; y < gridSize; y += b) {
      for (let x = 0; x < gridSize; x += b) {
        let occupied = false;
        for (let dy = 0; dy < b && y + dy < gridSize; dy++) {
          for (let dx = 0; dx < b && x + dx < gridSize; dx++) {
            if (grid[(y + dy) * gridSize + (x + dx)] === 1) {
              occupied = true;
              break;
            }
          }
...
export function computeThirdMomentBeta(
  hm: HarmonicMeasure,
  cluster: DlaCluster,
): ThirdMomentResult {
  const { mu } = hm;
...
export function runZ2Discharge(
  ...
  if (tm.nSites < 10) {
    ...
    note = `Too few boundary sites (${tm.nSites}) for reliable measurement. Increase nWalkers.`;
  } else if (tm.falsifierFires) {
    conjecture = "FALSIFIED";
    note = `|β − τ(3)| = ${tm.gap.toFixed(4)} > ${FALSIFIER_TOLERANCE} (tolerance). ` +

...
export function gearTable(): bigint[] {
  const t: bigint[] = [];
  for (let i = 0; i < 256; i++) t.push(mix(BigInt(i)));
  return t;
}

/** Deterministic test byte stream: byte[i] = mix(i) & 0xFF. */
export function genBytes(count: number): number[] {
  const out: number[] = [];
  for (let i = 0; i < count; i++) out.push(Number(mix(BigInt(i)) & 0xffn));
  return out;
}
...
export function chunkLengths(bytes: number[], min: number, avg: number, max: number): number[] {

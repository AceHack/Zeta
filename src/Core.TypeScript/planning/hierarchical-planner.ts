...
/**
 * Executes 2-Level Hierarchical Search over grid space.
 */
export function hierarchicalGridSearch(
  ...
  if (!coarseBlockPath) {
    return { totalStatesExplored: 0, plan: null };
  }
  ...
  for (let i = 0; i < coarseBlockPath.length; i++) {
    ...
    if (!subPlan) {
      return { totalStatesExplored, plan: null };
    }

    fullPlan.push(...subPlan);

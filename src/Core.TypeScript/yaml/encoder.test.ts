...
import { parse, type YamlValue } from "./dom";
...
const I = (n: bigint): YamlValue => ({ t: "Int", value: n });
const S = (s: string): YamlValue => ({ t: "Str", value: s });
const M = (entries: Array<[string, YamlValue]>): YamlValue => ({ t: "Map", entries });
const SEQ = (items: YamlValue[]): YamlValue => ({ t: "Seq", items });

function roundtrips(v: YamlValue): boolean {
...
function toJ(v: YamlValue): unknown {
  switch (v.t) {

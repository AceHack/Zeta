...
import {
  generateUpProjectionQuery,
  generateTimeTravelQuery,
  generateStreamCompositionQuery,
  formatAsOfSystemTime,
  CockroachUpProjectionSubstrate,
  type QueryExecutor,
  type PackageNode,
  type DependencyEdge,
} from "./cockroach-up-projection.ts";
...
describe("Substrate Execution with Mock Executor", () => {
  test("upsertPackage and project dispatch correct SQL and parameters", async () => {
    const executed: { sql: string; params?: readonly any[] }[] = [];
...
describe("CockroachDB Live Integration Tests", () => {
  test("creates schema, resolves dependencies, detects cycles and version conflicts", async () => {
    ...
    try {
      ...
      const nowResult = (await pool.query("SELECT now() as now")) as { rows: { now: Date }[] };

...
async function acceptN<TN, TF>(accepted: AsyncIterable<import("../model-backend/multiplexed-duplex-transport.ts").MuxChannel<TN, TF>>, n: number) {
  const out = [];
  ...
  test("INTERRUPT: 'stop' signal crosses lossy wire and halts the real persona participant", async () => {
    ...
    const turn: ModelTurn = () => {
      ...
      if (turnCalls === 1) {
        ...
        return gate.then(() => ({ ok: true as const, text: "", calls: [{ name: "long_running", callId: "c1", arguments: {} }] }));
      }
    ...
    const testSummoner: ISummon = {
      summon: async (_personaName, prompt, _opts) => {
        ...
        // Pump mesh to let handshake cross
        mesh.advance(100); 
    ...
    expect(result.raw).toContain("interrupted (feedback corner)");

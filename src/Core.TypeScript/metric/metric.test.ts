...
interface BloomGolden {
  bucketCount: number;
  probesPerLookup: number;
  keys: string[];
  table: string[];
}
interface CmsGolden {
...
test("Bloom no-false-negative on the golden keys", () => {
  ...
  const f = new BlockedBloomFilter(g.bucketCount, g.probesPerLookup);
  for (const k of g.keys) f.add(BigInt(k));
  for (const k of g.keys) expect(f.mayContain(BigInt(k))).toBe(true);
});

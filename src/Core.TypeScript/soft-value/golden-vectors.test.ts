...
import { resolve, observeResolve, type Weights } from "./soft-value";
...
const seed = seedJson as unknown as {
  resolve: { candidates: Weights; num: number; den: number; result: string | null }[];
  observeResolve: { prior: Weights; likelihood: Weights; num: number; den: number; result: string | null }[];
};

test("TS SoftValue agrees with the shared golden seed (decisions)", () => {
  for (const v of seed.resolve) expect(resolve(v.candidates, v.num, v.den)).toBe(v.result);

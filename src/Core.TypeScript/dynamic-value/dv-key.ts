...
export class DvKey {
  ...
  private constructor(value: Tagged, canonical: Uint8Array) {
    this.value = value;
    this.canonical = canonical;
  }

  /** Wrap a `Tagged` value as a comparable, content-addressed row key. */
  static ofValue(value: Tagged): DvKey {
  ...
  equals(other: unknown): boolean {
    if (!(other instanceof DvKey)) return false;

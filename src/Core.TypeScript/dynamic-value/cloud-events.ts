...
/**
 * **CloudEvents (CNCF v1.0) envelope over Tagged (DynamicValue) — the bus-envelope standard.**
 */
...
export function validate(
  ...
  if (!e.specversion) missing.push("specversion");
...
export function toDynamic(e: CloudEvent): Tagged {
...
export function ofDynamic(
  dv: Tagged
): { ok: true; value: CloudEvent } | { ok: false; error: string } {
  if (dv.t !== "obj") {

...
module ThresholdSignatureVerification =
    ...
    [<Literal>]
    let SigningDomain = "zeta.threshold-signature.v1"

    /// The canonical byte string a signer signs: `domain ‖ len(scope) ‖ scope ‖ len(payload) ‖ payload`
    /// with UTF-8 scope bytes and 4-byte **little-endian, unsigned** length prefixes. Length prefixing
    ...
    let signingBytes (scope: string) (payload: byte[]) : byte[] =
        ...
        let len (n: int) = BitConverter.GetBytes(uint32 n) |> fun b -> if BitConverter.IsLittleEndian then b else Array.rev b

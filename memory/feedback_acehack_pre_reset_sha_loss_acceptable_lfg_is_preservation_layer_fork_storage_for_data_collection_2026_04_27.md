...
### The architecture — two categories, not one

Aaron 2026-04-27 follow-up: *"decisions, research can likey just stay in shared lfg location instead of fork specific unless there is a good reason to split? maybe you thougt of somethign i didn't"*

Aaron is right. The earlier framing of "fork-storage locations" lumped together two distinct categories. They need separating.

**Category A: SHARED-on-LFG (project-wide, not fork-partitioned).**

These are inherently project-wide artifacts. Multiple forks can *contribute* (any fork can propose an ADR, contribute research findings, ferry cross-AI work) but the canonical store is one — the project doesn't have divergent decisions/research per fork.

- **`docs/DECISIONS/`** — ADRs (architectural decisions for the project, not per-fork).
- **`docs/research/`** — research history (project-wide knowledge).
- **`docs/aurora/`** — courier-ferry archive (cross-AI research, project-wide).
- **`docs/ROUND-HISTORY.md`** — round-close synthesis (project-wide narrative).
- Factory-wide `memory/*.md` — substrate/learnings everyone shares.

**Category B: Per-fork / per-maintainer-pair (naturally partitioned).**

These capture experience or data tied to a specific fork or maintainer-agent pair. Different pairs have different PR conversations, different tick rhythms, different budget profiles. Partitioning is intrinsic.

- **`docs/pr-preservation/`** — PR conversation archives. Each fork's PRs have separate review threads (AceHack PR #45 vs LFG PR #645 are different PRs); per-fork partitioning is the data shape.
- **`docs/hygiene-history/`** — tick-history + drain-logs from autonomous-loop ticks. Each maintainer-agent pair has its own tick rhythm.
- **`docs/budget-history/`** — cost-data snapshots. Per-maintainer-pair spending profile.
- **`memory/persona/<name>/`** — persona notebooks. Per-persona, which means per-pair-context.
- Commit messages and PR titles/bodies on LFG side — git-native record (PR-specific, captured per fork via paired-sync).

### How to decide: "is this fork-specific or project-wide?"

Quick test — would two different maintainer-agent pairs working on the same project produce *different* versions of this artifact?

- If yes → per-fork partition (Category B). Examples: their PR conversations differ, their tick rhythms differ, their budget profiles differ.
- If no → shared (Category A). Examples: the project's architectural decisions don't differ per pair; the project's research findings don't differ per pair.

### The collective-training claim still holds

Aaron's load-bearing framing: *"all forks ... can keep it's data on LFG too so everyone can train from it and learn form it."*

Both categories serve collective training/learning, just via different mechanisms:

- **Category A (shared)**: everyone reads the same canonical decisions/research/aurora data — the corpus is unified by design.
- **Category B (per-fork)**: per-fork data is collective-readable even though write-partitioned — a future contributor can read all forks' PR-preservation drain-logs / cost-history / tick-history to learn from collective experience, while each fork's writes only land in its own partition.

The collective-training architecture works either way; the cleaner split just makes "where does my new artifact land?" easier to decide.

### Original mistaken framing (preserved for lineage)

The first version of this memory listed `docs/DECISIONS/`, `docs/research/`, `docs/aurora/` under "fork-storage locations" without distinguishing them from genuinely fork-partitioned artifacts. Aaron flagged the confusion. Decisions, research, and aurora are project-wide and live in one shared place; PR conversations, cost data, tick-history, and persona notebooks are per-pair and partitioned. The two-category split above is the corrected framing.
